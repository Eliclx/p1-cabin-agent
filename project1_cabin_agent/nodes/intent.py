...
from project1_cabin_agent.nodes.models import IntentOutput
from project1_cabin_agent.nodes.models import STRONG_COREFERENCE
...
from project1_cabin_agent.nodes.intent_carry import _try_carry_over, _needs_context
from project1_cabin_agent.nodes.intent_drift import _detect_context_bleeding
from project1_cabin_agent.nodes.intent_ambiguity import _detect_ambiguity
...
3. 槽位值只能从用户输入提取，禁止从历史回复提取

【上轮结果引用】：
dialogue_context 含上轮工具结果，仅含指代词时参考。指代时直接填 extracted_slots，不创建额外搜索任务，depends_on=[]。询问上轮结果细节（"第2个是什么""最远的是哪个"）用 direct_answer，slots留空。禁止用 direct_answer 编造需实时数据的答案。指令模糊/缺少操作对象时用 clarify 列候选。超能力范围用 no_support。
...
def intent_classifier(state: CabinAgentState) -> dict:
    ...
    try:
        ...
        return _create_fallback_result("json_error")

    except Exception as e:
        logger.error(f"[意图识别] ❌ 异常: {e}")
        return _create_fallback_result("unknown_error")
