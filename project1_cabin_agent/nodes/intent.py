...
def _can_use_edge(user_input: str, active_frames: list) -> bool:
    ...
    # 2b. "最近的" + 追问模式 → 需要上下文
    _FOLLOWUP_AFTER_NEAREST = re.compile(
        r"最近的.{0,4}(有多远|多远|是哪个|哪个|在哪里|在哪|怎么样|多少钱|电话|评价)"
    )
...
INTENT_PROMPT = (
    """你是车载语音助手意图理解中枢。分析用户输入，理解情绪需求并响应（如用户冷热感则考虑调空调），高情商处理。返回纯 JSON。
...
【条件分支】：用户说"有的话就X""天气好就X"等条件触发时，被触发的任务加 condition 字段。condition.rules 引用上游 task 的 tool_result 字段。op 可选: eq/neq/gt/gte/lt/lte/in/not_in/is_empty/is_not_empty。fail_msg 写条件不满足时的简短回复。无条件语义时不要生成 condition。
...
# 条件分支示例 prompt 片段（注入到 CLOUD_PROMPT 之后，不修改主 prompt）
CONDITION_EXAMPLE = """\n\n【条件分支示例】：
用户:"查下附近有没有充电站，有的话导航过去"
→ task_0: search_poi(keyword="充电站", depends_on=[])
→ task_1: navigate(depends_on=["task_0"], condition={{"logic":"AND","rules":[{{"source":"task_0","field":"count","op":"gt","value":0}}],"fail_msg":"附近没有找到充电站"}})
...
def intent_classifier(state: CabinAgentState) -> dict:
    ...
    if carried:
        logger.info("[意图识别] Carry-Over 命中，跳过 LLM")
        return {
            "sub_tasks": [carried],
            "is_complex": False,
            "task_results": None,
    ...
    if episodic_context:
        ...
        logger.info(
            f"[意图识别] L1.5 行程记忆命中，{len(episodic_context['raw'])}条，注入上下文"
        )
    ...
    from project1_cabin_agent.edge_model import (
        EDGE_ENABLED,
        edge_model_infer,
        edge_result_to_subtask,
    )

    if (
        EDGE_ENABLED
        and _can_use_edge(user_input, active_frames)
        and not oos_flag
        and not cross_domain_flag
    ):
    ...
    llm = get_llm("fast", temperature=0.1, timeout=60)
    dialogue_ctx = state.get("dialogue_context", {})
    ctx_text = (
        json.dumps(dialogue_ctx, ensure_ascii=False, indent=2)
        if dialogue_ctx
        else "（无）"
    )
    ...
    try:
        logger.info(
            f"[意图识别] 调用 LLM，user_input={user_input}，needs_ctx={needs_ctx}"
        )
        ...
        for i, st in enumerate(results.sub_tasks):
            ...
            if st.condition:
                logger.info(f"[子任务{i + 1}] condition={st.condition}")
        ...
        for t in sub_tasks_data:
            ...
            if invalid:
                logger.warning(
                    f"[depends_on校验] {t.get('task_id')} 的依赖 {invalid} 不在当前 sub_tasks 中，已清空"
                )
        ...
        if episodic_context:
            ...
            sub_tasks_data = guard_episodic_extraction(
                sub_tasks_data, episodic_context["raw"]
            )
        ...
        if len(new_frames) >= MAX_PENDING_FRAMES:
            ...
            logger.warning("[active_frames] pending 帧超限，丢弃最旧的")
        for task in sub_tasks_data:
            missing = [
                s
                for s in task.get("required_slots", [])
