...
class AcControlSlots(BaseModel):
    ...
    temperature: Optional[float] = Field(
        default=None, ge=16, le=32, description="目标温度(℃)，范围16-32"
    )
    mode: Optional[Literal["cool", "heat", "auto"]] = Field(
        default=None, description="模式: cool(制冷)/heat(制热)/auto(自动)"
    )
    fan_level: Optional[int] = Field(
        default=None, ge=1, le=5, description="风速档位1-5"
    )
...
class WindowControlSlots(BaseModel):
    ...
    percent: Optional[int] = Field(
        default=None, ge=0, le=100, description="开合百分比(0-100)"
    )
...
class LightControlSlots(BaseModel):
    ...
    action: Literal["on", "off", "adjust"] = Field(description="操作: on/off/adjust")
    target: Optional[Literal["cabin", "reading", "ambient"]] = Field(
        default=None, description="灯光类型: cabin(车内)/reading(阅读)/ambient(氛围)"
    )
    brightness: Optional[int] = Field(
        default=None, ge=0, le=100, description="亮度0-100"
    )
...
class SeatControlSlots(BaseModel):
    ...
    heat_level: Optional[int] = Field(
        default=None, ge=1, le=3, description="加热档位1-3"
    )
...
CLIMATE_MEMORY: dict[str, dict] = {
    "ac_control": {
        "log": False,  # 控制类不记
        "l3_keys": {"temperature": "ac_temperature"},  # 但写入 L3 偏好
    },
    "window_control": {"log": False},
    "light_control": {"log": False},
    "seat_control": {"log": False},
    "cabin_query": {"log": False},
}
