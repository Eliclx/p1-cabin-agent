...
def format_ac_control(tool_result: dict) -> CabinAction | None:
    ...
    if action == "on":
        return CabinAction(domain="climate", intent="ac_control",
                           command="ac_on",
                           params={"temperature": tool_result.get("temperature", 24)})
    elif action == "off":
        return CabinAction(domain="climate", intent="ac_control", command="ac_off")
    elif action == "adjust":
        params = {k: tool_result[k] for k in ("temperature", "mode", "fan_level") if k in tool_result}
        return CabinAction(domain="climate", intent="ac_control", command="ac_set", params=params)
    return None
...
def format_window_control(tool_result: dict) -> CabinAction | None:
    ...
    if action == "open":
        return CabinAction(domain="climate", intent="window_control",
                           command="window_open", params={"target": target})
    elif action == "close":
        return CabinAction(domain="climate", intent="window_control",
                           command="window_close", params={"target": target})
    elif action == "adjust":
        return CabinAction(domain="climate", intent="window_control",
                           command="window_set",
...
def format_light_control(tool_result: dict) -> CabinAction | None:
    ...
    if action == "on":
        ...
        return CabinAction(domain="climate", intent="light_control", command="light_off")
    elif action == "adjust":
        params = {k: tool_result[k] for k in ("target", "brightness") if k in tool_result}
        return CabinAction(domain="climate", intent="light_control", command="light_set", params=params)
    return None
...
def format_seat_control(tool_result: dict) -> CabinAction | None:
    action = tool_result.get("action", "")
    if action == "heat_on":
        return CabinAction(domain="climate", intent="seat_control",
                           command="seat_heat_on",
                           params={"heat_level": tool_result.get("heat_level", 1)})
    elif action == "heat_off":
        return CabinAction(domain="climate", intent="seat_control", command="seat_heat_off")
    elif action == "ventilate_on":
        return CabinAction(domain="climate", intent="seat_control", command="seat_vent_on")
    elif action == "ventilate_off":
        return CabinAction(domain="climate", intent="seat_control", command="seat_vent_off")
    return None
