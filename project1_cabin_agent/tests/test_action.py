...
class TestClimateAction:
    def test_ac_on(self):
        result = {"status": "success", "intent": "ac_control", "action": "on", "temperature": 24}
    ...
    def test_ac_set(self):
        result = {"status": "success", "intent": "ac_control", "action": "adjust",
                  "temperature": 22, "mode": "cool", "fan_level": 3}
        action = format_action("climate", "ac_control", result)
        assert action.command == "ac_set"
        assert action.params == {"temperature": 22, "mode": "cool", "fan_level": 3}

    def test_window_open(self):
        result = {"status": "success", "intent": "window_control", "action": "open", "target": "driver"}
    ...
    def test_window_close(self):
        result = {"status": "success", "intent": "window_control", "action": "close", "target": "all"}
        action = format_action("climate", "window_control", result)
        assert action.command == "window_close"

    def test_window_adjust(self):
        result = {"status": "success", "intent": "window_control", "action": "adjust",
                  "target": "passenger", "percent": 50}
        action = format_action("climate", "window_control", result)
    ...
    def test_seat_heat(self):
        result = {"status": "success", "intent": "seat_control", "action": "heat_on", "heat_level": 3}
...
class TestMapAction:
    def test_navigate(self):
        result = {"success": True, "data": {
            "distance": 300, "duration": 210, "tolls": 165,
    ...
    def test_weather(self):
        result = {"success": True, "data": {
            "city": "成都", "weather": "晴", "temperature": "28", "date": "今天",
        }}
...
class TestMediaAction:
    def test_play(self):
        result = {"status": "success", "intent": "media_control", "action": "play",
                  "query": "周杰伦", "artist": "周杰伦"}
    ...
    def test_volume(self):
        result = {"status": "success", "intent": "media_control", "action": "set_volume", "volume": 80}
        action = format_action("media", "media_control", result)
...
class TestCabinActionSerialization:
    def test_to_dict(self):
        a = CabinAction(domain="climate", intent="ac_control",
                        command="ac_on", params={"temperature": 24})
        d = a.to_dict()
        assert d == {
            "domain": "climate",
            "intent": "ac_control",
    ...
    def test_from_dict(self):
        d = {"domain": "map", "intent": "navigate",
             "command": "start_nav", "params": {"distance_km": 300}}
        a = CabinAction.from_dict(d)
        assert a.domain == "map"
        assert a.command == "start_nav"

    def test_roundtrip(self):
        original = CabinAction(domain="media", intent="media_control",
                               command="media_play", params={"query": "周杰伦"})
